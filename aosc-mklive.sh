...
if [[ "$ARCH" = "amd64" || "$ARCH" = "arm64" || "$ARCH" = "loongarch64" ]]; then
...
# FIXME: Flatten the ISO image  as LoongArch firmware do not seem to handle
# GPT hybrid images well (especially those with FAT12/16 filesystems and long
# filenames, such as the efi.img generated by grub-mkrescue).
#
# It's probably one of the options... Let's try to upstream a fix some time.
if [[ "$ARCH" = "loongarch64" ]]; then
	echo "Extracting ISO content ..."
	xorriso -osirrox on -indev "$ISO_NAME" -extract / iso/

	echo "Dropping unneeded El Torito data and ESP image ..."
	rm -v iso/{boot.catalog,efi.img}

	echo "Re-generating ISO ..."
	xorriso \
		-no_rc \
		-as mkisofs \
		-iso-level 3 \
